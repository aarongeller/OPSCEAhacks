...
% jumpto allows video to start at a later point, __ seconds ahead (default 0)
...
if ~exist('setvisible','var')||isempty(setvisible)
    setvisible='on';
end 
...
LL=nan(size(d)); 
for i=1:size(d,2)-L
    LL(:,i)=sum(abs(diff(d(:,i:i+L),1,2)),2); 
end
...
vidperiodidx=cast(round(S.VIDperiod(1)*sfx+1):S.VIDperiod(2)*sfx, 'uint32');
d=d(:,vidperiodidx); 
...
for i=cast(frametimpoints, 'int32'); 
    if i==jumpto+2*S.fram; 
        ...
        disp(['Length of data is ' num2str(ntp/sfx) ' sec']); 
        disp(datetime); 
    ...
    switch upper(plottype{j,1})
      case 'ECOG' %plot the raw data
            inds = cast((i-S.marg+1):(i-S.marg+1)+sfx*S.iceegwin, 'uint32');
            dtoplot=d(nns,inds);
            tstoplot=ts(inds);
            ...
            fill(ones(1,4)*ts(i)+[0 S.llw S.llw 0],[.5 .5 -nch-1.25 -nch-1.25],[.4 .4 .4],'facealpha',.25,'edgealpha',1); hold off; % overlay transform window
              ...
              % text(i/sfx-.01+S.llw*.36,2,'v'); 
              % text(repmat(i/sfx+.01705+S.llw*.4,1,4),2.5:1:5.5,{'|','|','|','|'}) %draws an arrow pointing to the transform window
      ...
      case 'SURFACE' % plotting surfaces only
        ...
        axis(axislim); 
        % if strcmpi(viewangle{j},'i') || strcmpi(viewangle{j},'s') || ...
        %         strcmpi(viewangle{j},'a') || strcmpi(viewangle{j},'p')
        %     if ~strcmpi(pt,'NO181')
      ...
      case 'DEPTH' %plot depth electrode with parallel slice (plus surface behind it)
        ...
        elseif ~isempty(eNID)
            ...
            axis(axislim); 
            zoom(pltzoom(j)); % apply the specified zoom for that this view (usually similar for all depths but can depend on angle of slice, position of electrodes, etc)
